...
def block_ap(
    ...
    # ADDED: Load sensitivity scores if provided
    ...
    if hasattr(args, 'sensitivity_file') and args.sensitivity_file and os.path.exists(args.sensitivity_file):
        logger.info(f"Loading sensitivity scores from {args.sensitivity_file}")
        with open(args.sensitivity_file, 'r') as f:
            sensitivity_data = json.load(f)
        sensitivity_scores = torch.tensor(sensitivity_data['sensitivity_scores'])
        logger.info(f"Loaded sensitivity scores for {len(sensitivity_scores)} layers")
        logger.info(f"Sensitivity range: {sensitivity_scores.min():.4f} to {sensitivity_scores.max():.4f}")
    ...
    for block_index in range(len(layers)):
        ...
        # ADDED: Log sensitivity if available
        ...
        if args.epochs > 0:
            ...
            # ADDED: Adaptive epochs based on sensitivity (OPTIONAL)
            if sensitivity_scores is not None and hasattr(args, 'adaptive_epochs') and args.adaptive_epochs:
                ...
                logger.info(f"Using adaptive epochs: {adaptive_epochs} (base: {args.epochs})")
            else:
