"""
Research Script for Llama 3.x: Sensitivity-Guided Mixed-Precision Quantization

This version includes compatibility fixes for Llama 3.1 rope_scaling configuration.

Usage:
    python main_research_llama3.py --model meta-llama/Llama-3.1-8B \
        --sensitivity_file ./sensitivity_results_llama_3.1_8b.json \
        --use_mixed_precision --mpq_strategy aggressive \
        --target_avg_bits 4.0 \
        --real_quant --train_size 128 --val_size 16 \
        --quant_lr 1e-4 --weight_lr 1e-5 \
...
from quantize.block_ap_research import block_ap
...
def load_llama3_config(model_name_or_path):
    ...
    Llama 3.1 uses extended rope_scaling format that older transformers versions don't support.
    This function handles the conversion to the expected format.
    
    CRITICAL: We now DISABLE rope_scaling entirely to avoid compatibility issues
    that break positional encoding. The model will use standard RoPE within its
    original 8K context window (we use 2048 seqlen anyway).
    """
    try:
        config = AutoConfig.from_pretrained(model_name_or_path)
        # Disable rope_scaling to avoid compatibility issues
        if hasattr(config, 'rope_scaling') and config.rope_scaling is not None:
            print(f"[INFO] Disabling rope_scaling to avoid compatibility issues")
            print(f"[INFO] Original rope_scaling: {config.rope_scaling}")
            config.rope_scaling = None
            print(f"[INFO] Model will use standard RoPE (sufficient for 2048 seqlen)")
        return config
        ...
        if 'rope_scaling' in config_dict:
            print(f"[INFO] Original rope_scaling: {config_dict['rope_scaling']}")
            print(f"[INFO] Removing rope_scaling entirely to avoid compatibility issues")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Research (Llama 3.x): Sensitivity-Guided Mixed-Precision Quantization")
    ...
    # Training parameters
    parser.add_argument("--quant_lr", type=float, default=5e-5)  # Reduced to prevent gradient explosion
    parser.add_argument("--weight_lr", type=float, default=5e-6)  # Reduced to prevent gradient explosion
    parser.add_argument("--min_lr_factor", type=float, default=20)
    parser.add_argument("--clip_grad", type=float, default=1.0)  # Increased for stronger clipping
    ...
    parser.add_argument("--target_avg_bits", type=float, default=4.5,
                        help="Target average bit-width for MPQ (recommend 4.5-5.0 for quality)")
    ...
    parser.add_argument("--target_size_mb", type=float, default=None,
                        help="Target model size in MB for budget optimization")
    
    # Ablation study flags
    parser.add_argument("--ablation", type=str, default=None,
                        choices=["mpq_only", "sgra_only", "qbo_only", "all"],
    ...
    logger.info(f"  QBO enabled: {args.target_size_mb is not None}")
    ...
    if args.resume_quant:
        ...
        # Load FP16 model with Llama 3 compatibility
